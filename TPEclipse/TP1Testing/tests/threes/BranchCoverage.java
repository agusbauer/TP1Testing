...
public class BranchCoverage {
	...
	@Test//(timeout = 2000)
    public void firstRowWithFreeTile() {
        ThreesBoard board = new ThreesBoard(); 
        for (int i = 0; i < ThreesBoard.ROWS; i++) {
            for (int j = 0; j < ThreesBoard.COLUMNS; j++) {
                board.set_tile(i, j, 1); 
        ...
        assertTrue(threes.getBoard().get_tile(0, 1).getValue() == 1);
        assertTrue(threes.getBoard().get_tile(0, 2).getValue() == 3);
        assertTrue(threes.getBoard().get_tile(0, 3).getValue() == 1);
    }
